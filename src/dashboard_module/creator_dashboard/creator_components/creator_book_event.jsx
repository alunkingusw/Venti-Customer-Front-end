...
const Creator_book_event = () => {
    ...
    const handleBuyTicket = async (values) => {
        if (totalAmount === 0) {
            Error('Please select at least one ticket.');
            return;
        }

        const ticketTypes = ['Early_bird', 'Regular', 'VIP', 'VVIP'];
        const ticketData = ticketTypes
            .filter(type => tickets[type] > 0) // Only include ticket types with a quantity > 0
            .map(type => ({
                type: type.replace('_', ' '),
                quantity: tickets[type],
                total_price: calculateTotal(type)
            }));
    
        // Prepare the payload
        const payload = {
            Email: email, // Ensure 'email' is retrieved from form input
            EventId: id, // Event ID from useParams
            eventName: event.eventName, // Event name from state
            ticket_type: ticketData,
            total_price: totalAmount,
            payment_info: {
                card_number: cardNumber, // Ensure 'cardNumber' is retrieved from form input
                CVV: cvv, // Ensure 'cvv' is retrieved from form input
                card_expiry: cardExpiry, // Ensure 'cardExpiry' is retrieved from form input
                cardType: 'Visa', // You can adjust this based on your form input or selection
                Name_on_Card: cardHolder // Ensure 'cardHolder' is retrieved from form input
            }
        };
        try {
            const { data } = await EndPoints.tickects.purchase(payload)
            console.log(data)
        } catch (error) {
            console.log(error)
        }
                                ...
                                <div className="grid grid-cols-2 gap-6">
                                    <div className="col-span-2 sm:col-span-1">
                                        ...
                                        <input
                                            ...
                                            className="w-full py-3 px-4 border border-gray-400 rounded-lg focus:outline-none focus:border-blue-500 dark:bg-transparent"
                                        />
                                    </div>
                                    <div className="col-span-2 sm:col-span-1">
...
export default Creator_book_event;